...
path = 'C:/Users/adlogan/Dropbox/Projects/Frequency Conversion/Layout/KLayout/DFG200/Inverse Design Chip 2021 06 07/'
path_test = path + 'Devices/'
path_array = path + 'Arrays/'

filename_full = 'DFG200_SHG_Inv_v2'
...
for lambda_type in ['1','3','1ir','2ir','2nir']:
	grating_cells['SHG'+lambda_type] = {}
	for shape_type in c.gr_spec['SHG'][lambda_type]['shape']:
		filename = 'grating_SHG' + lambda_type + '_' + shape_type
		gr_path = ['SHG',lambda_type]
		...
		grating_cells['SHG'+lambda_type][shape_type] = layout.create_cell(filename)
		
		for shape in grating[0]:
			grating_cells['SHG'+lambda_type][shape_type].shapes(GaP).insert(shape)
		
		grating_cells['SHG'+lambda_type][shape_type].write(path_test + filename + '.gds')
...
for lambda_type in grating_cells:
    ...
    for shape_type in grating_cells[lambda_type]:
        ...
        in_pt = origin;
        (shapes, last_pt) = p.waveguide(origin, 45, w_wg, (4000+12400+1000)*c.scale)
        
        p.add(shapes, p.wg_bez_p2p(last_pt, out_pt, 45, 315, w_wg, L_bz, 360))
        
        gr_out_pt = p.add(shapes,p.waveguide(out_pt, 315, w_wg, 2000*c.scale))[0]
        ...
        for resonator_type in resonator_cells:
            ...
            in_pt = origin;
            (shapes, last_pt) = p.waveguide(origin, 45, w_wg, 1000*c.scale)
            last_pt = p.add(shapes,p.wg_adapt(last_pt, 45, w_wg, w_res, 2000*c.scale))[0]
            res_pt = last_pt
            res_out_pt = resonator_end_trans[resonator_type]*res_pt
            
            
            last_pt = p.add(shapes,p.wg_adapt(res_out_pt, 45, w_res, w_wg, 2000*c.scale))[0]
            p.add(shapes, p.wg_bez_p2p(last_pt, out_pt, 45, 315, w_wg, L_bz, 360))
...
for lambda_type in grating_cells:
    ...
    for shape_type in grating_cells[lambda_type]:
        ...
        filename_test = 'test_array_' + lambda_type + '_' + shape_type
        
        current_cell = layout.create_cell(filename_test)
        current_label = layout.create_cell(filename_test+'_label');
        
        L.label_insert(current_label, digit_cells, c.text_spec['d'], '{:02}'.format(tick))
        ...
        current_cell.insert(pya.CellInstArray(current_label.cell_index(), pya.Trans(0,False, x_position*array_dx, 14000*c.scale)))
        
        array_cells[lambda_type][shape_type] = current_cell
        current_cell.write(path_array + filename + '.gds')
        top.insert(pya.CellInstArray(current_cell.cell_index(), pya.Trans(0,False, -50000*c.scale, tick*50000*c.scale)))
...
for lambda_type in grating_cells:
    ...
    for shape_type in grating_cells[lambda_type]:
        ...
        array_cells[lambda_type][shape_type] = current_cell
        current_cell.write(path_array + filename + '.gds')
        top.insert(pya.CellInstArray(current_cell.cell_index(), pya.Trans(0,False, 0, array_num*50000*c.scale)))
