...
path = 'C:/Users/adlogan/Dropbox/Projects/Frequency Conversion/Lumerical/Projects/Photonics Design/GaP on SiN/GaP on SiN with HSQ/GaPSiNOx250_SHG1_2021_01_14/'
path_test = path + 'Devices/'
path_array = path + 'Arrays/'

filename_full = 'GaPSiNOx250_SHG2'
...
ring_mod = {'SR003_R7770_W940':range(-40,40,16)}#[0]}#
...
for ring_type in ['1550TE']:
	...
	for cp_type in cp_Q_single[ring_type]:
		...
		for mod_type in range(1):
			...
			if 'in' in c.layout_spec['cp_ring'][ring_type]['lr_v1']['cp']['inp'][ring_type]:
			...
			if 'out' in c.layout_spec['cp_ring'][ring_type]['lr_v1']['cp']['inp'][ring_type]:
			...
			if 'in' in c.layout_spec['cp_ring'][ring_type]['lr_v1']['cp']['drop'][ring_type]:
				current_cell.insert(pya.CellInstArray(grating_cells[ring_type]['ell'].cell_index(), pya.Trans(cp_pts[1][0][1],False,cp_pts[1][0][0].x,cp_pts[1][0][0].y)))
			if 'out' in c.layout_spec['cp_ring'][ring_type]['lr_v1']['cp']['drop'][ring_type]:
				current_cell.insert(pya.CellInstArray(grating_cells[ring_type]['ell'].cell_index(), pya.Trans(cp_pts[1][1][1],False,cp_pts[1][1][0].x,cp_pts[1][1][0].y)))
...
for ring_type in ['637TM']:
	cp_ring_single_cells[ring_type] = {}
	cp_ring_single_cell_pts[ring_type] = {}
	ring_path = [ring_type,'lr_v2']
	wg_path = [[ring_type],[ring_type]]
	layout_path = ['cp_ring',ring_type,'lr_v2']
	layout_cp_path = [['std',ring_type],['drop',ring_type]]
	for cp_type in cp_Q_single[ring_type]:
		cp_ring_single_cells[ring_type][cp_type] = {}
		cp_ring_single_cell_pts[ring_type][cp_type] = {}
		ring_cp_path = [[ring_type,cp_type[0]],[ring_type,cp_type[0]]]
		for mod_type in range(1):
			...
			L.label_insert(current_cell, digit_cells, c.text_spec['d'], c.ring_spec[ring_type]['lr_v2']['label'], pt_label_1[0])
			
			if 'in' in c.layout_spec['cp_ring'][ring_type]['lr_v2']['cp']['std'][ring_type]:
				current_cell.insert(pya.CellInstArray(grating_cells[ring_type]['ell'].cell_index(), pya.Trans(cp_pts[0][0][1],False,cp_pts[0][0][0].x,cp_pts[0][0][0].y)))
			if 'out' in c.layout_spec['cp_ring'][ring_type]['lr_v2']['cp']['std'][ring_type]:
				current_cell.insert(pya.CellInstArray(grating_cells[ring_type]['ell'].cell_index(), pya.Trans(cp_pts[0][1][1],False,cp_pts[0][1][0].x,cp_pts[0][1][0].y)))
			
			if 'in' in c.layout_spec['cp_ring'][ring_type]['lr_v2']['cp']['drop'][ring_type]:
				current_cell.insert(pya.CellInstArray(grating_cells[ring_type]['ell'].cell_index(), pya.Trans(cp_pts[1][0][1],False,cp_pts[1][0][0].x,cp_pts[1][0][0].y)))
			if 'out' in c.layout_spec['cp_ring'][ring_type]['lr_v2']['cp']['drop'][ring_type]:
				current_cell.insert(pya.CellInstArray(grating_cells[ring_type]['ell'].cell_index(), pya.Trans(cp_pts[1][1][1],False,cp_pts[1][1][0].x,cp_pts[1][1][0].y)))
...
for ring_type in cp_ring_single_cells:
	...
	for cp_type in cp_ring_single_cells[ring_type]:
		...
		x_num = c.array_spec['cp_ring'][ring_type]['lr_v1']['drop']['x_num']
		
		for dy in range(math.floor(rep_single/x_num)):
			for dx in range(x_num):
				array_trans = pya.Trans(0,False,dx*c.array_spec['cp_ring'][ring_type]['lr_v1']['drop']['x'][0]  + dy*c.array_spec['cp_ring'][ring_type]['lr_v1']['drop']['y'][0],dx*c.array_spec['cp_ring'][ring_type]['lr_v1']['drop']['x'][1]  + dy*c.array_spec['cp_ring'][ring_type]['lr_v1']['drop']['y'][1])
